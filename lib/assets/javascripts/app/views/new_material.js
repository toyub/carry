(function(win, doc, Backbone, $$MIS){
  var NewMaterialView = Backbone.View.extend({
    events: {
      ...
      'change #material_root_category': 're_render_subs',
    ...
    error_handle: function(el, error){
      ...
      var span_height = 39;
      console.log(position, el[0].getBoundingClientRect())

      x = $("<div class='field_error field_down "+ error.attr +"'></div>").html(error.msg)
      x.css({
        top: (position.top - $('.new_products').position().top - el[0].getBoundingClientRect().height) + 'px',
