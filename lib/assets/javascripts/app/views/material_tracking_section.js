(function(win, doc, Backbone, $$MIS){
  ...
  var section_tmpl = function(model){
    if(!_section_tmpl){
      _section_tmpl = _.template($("#section_tmpl").html());
    }
  ...
  var calc_delay = function(time, time_unit){
    var one_hour = 3600;
    var one_day = one_hour * 24;
    switch(time_unit){
      case 'hour' : return time * one_hour;
      case 'day'  : return time * one_day;
      case 'week' : return time * one_day * 7;
      case 'month': return time * one_day * 30;
      case 'year' : return time * one_day * 365;
      default: throw new Error('argument "time_unit" must be one of ["hour", "day", "week", "month", "year", but get '+time_unit);
    }
  }
  ...
  var MaterialTrackingSectionView = Backbone.View.extend({
    ...
    submit: function(){
      ...
      var time_unit = this.model.get('time_unit');
      var delay = calc_delay(time, time_unit);
      this.model.set('delay', delay);
      this.commit();
    },
    commit: function(){
      this.model.commit();
