...
$(document).on("drop", "li.js-station", function(e){
    ...
    if(data){
      var mechanicDispatched = true
      var order = JSON.parse(data);
      var tip = order.state == 'queuing' ? '确认开始此流程？' : '是否确定结束该流程？';
      $.ajax({
        url: "/xianchang/store_orders/" + order.id + "/check_mechanic",
        method: "GET",
        async: false
      }).done(function(data){
        mechanicDispatched = data.status;
      })
      if(!mechanicDispatched){
        ZhanchuangAlert("请先为订单中的流程分配技师!");
        return false;
      }
