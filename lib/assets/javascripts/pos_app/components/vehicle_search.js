(function(win, doc, $, $$MIS){
  ...
  var editVehicleHandler = {
    saveVehicleSuccess: function(response){
      vehicleExist(response.id);
    }
  }

  var editCustomerHandler = {
    saveCustomerSuccess: function(response){
      customerFetch(vehicleAndCustomerHandler.current_customer.id);
    }
  }
  ...
  var vehicleAndCustomerHandler = {
    ...
    shwoVehicleEssentials: function(){
      var html = vehicle_essentials_tmpl({vehicle: this.current_vehicle.essentials(), customer: this.current_customer.essentials()});
  ...
  function editVehicle(){
    ...
    if(vehicleAndCustomerHandler.current_vehicle.isNew()){
      ...
      Mis.Views.editVehicle(vehicleAndCustomerHandler.current_vehicle.attributes, editVehicleHandler);
    }
  }
  ...
  function editCustomer(){
    ...
    if(vehicleAndCustomerHandler.current_customer.isNew()){
      ...
      Mis.Views.editCustomer(vehicleAndCustomerHandler.current_customer.attributes, editCustomerHandler);
    }
  }
  ...
  function editOrderInit(vehicle_id){
    vehicleExist(vehicle_id);
  }
  ...
  function VehiclesController(){/*需要在此做一些初始化的工作*/
    ...
    this.editOrderInit = editOrderInit;
  }
