(function(win, doc, $, $$MIS){
  ...
  var getCustomerName = function(){
    ...
    current_customer.set('last_name', last_name);
    current_customer.set('first_name', first_name);
  ...
  var addVehicleToCustomer = function(customer){/*TIP: vehicle(license entered but not found) and customer got */
    ...
    current_vehicle = new Mis.Models.StoreVehicle({license_number: getLicenseNumber()});
    current_customer.id = customer.id;
    current_customer.fetch();
  }
  ...
  function showAddVehicleForm(){
    new Mis.Views.NewCustomerView({customer: current_customer, license_number: getLicenseNumber(), phone_number: getPhoneNumber()}, newCustomerFormhandle);
  }
