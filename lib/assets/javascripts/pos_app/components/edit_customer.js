(function(win, Mis){
  var tmpl = JST["pos/orders/customer_detail"];
  var url = '/api/store_customer_entities/';
  var editCustomerHandler = null;

  Mis.Views.editCustomer = function(customer, handler) {
    editCustomerHandler = handler;
    if(customer.id){
      $.get(url + customer.store_customer_entity_id, function(response){
        var entity = response;
        entity.url = url + customer.store_customer_entity_id
  ...
  $(document).on('submit', '.improve_crm_wrap form', function(evt){
    ...
    $.ajax({
      ...
      success: function(response){
        editCustomerHandler.saveCustomerSuccess(response);
        ZhanchuangAlert("客户信息更新成功!");
        $('.js-select-items').trigger('click');
      },
      error: function(){
