(function(){
  ...
  var check_phone_number = function(phone_number){
    $.get('/api/crm/customers/check', {phone_number: phone_number}, function(response, status_str, xhr, undef){
      if(response.success){
        ...
        $('.vehicle_info [name="customer_name"]').val(response.customer.full_name)
      }else{
  ...
  Mis.SetVehicle = function(){
    ...
    $(".vehicle_info .js-phone-number").on('change', function(){
      if(!!this.value){
    ...
    $('#without-license-number').on('change', function(){
      if(this.checked){
        $('.vehicle_info .js-license-number').val('暂无牌照');
        $('.vehicle_info .js-license-number').attr('readonly', true);
      }else{
        $('.vehicle_info .js-license-number').val('');
        $('.vehicle_info .js-license-number').removeAttr('readonly');
      }
    });
