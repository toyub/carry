(function(win, Mis){
  var tmpl = JST["pos/orders/vehicle_detail"];
  var editVehicleHandler = null;
  Mis.Views.editVehicle = function(vehicle, handler) {
    editVehicleHandler = handler;
  ...
  $(document).on('submit', '#js-update-vehicle', function(evt){
    ...
    $.ajax({
      ...
      success: function(response, status_str, xhr, undef){
        editVehicleHandler.saveVehicleSuccess(response);
