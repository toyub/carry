Mis.Views.PosOrderIndexView = Backbone.View.extend({
  ...
  events: {
    ...
    'click .js-toggle-vehicle-condition': 'toggleVehicleCondition',
    'click .js-toggle-vehicle-details': 'toggleVehicleDetails',
    'click .js-customer-edit-cancel': 'selectItems',
    'click .js-vehicle-edit-cancel': 'selectItems'
  },
  initialize: function(){
  ...
  toggleVehicleCondition: function(evt){
    var $btn = $(evt.target);
    if($btn.hasClass('fa-angle-double-up')){
      this.$el.find('.vehicle_condition_info').hide();
      $btn.addClass('fa-angle-double-down');
      $btn.removeClass('fa-angle-double-up');
    }else{
      this.$el.find('.vehicle_condition_info').show();
      $btn.addClass('fa-angle-double-up');
      $btn.removeClass('fa-angle-double-down');
    }
  },

  toggleVehicleDetails: function(evt){
    var $btn = $(evt.target);
    if($btn.hasClass('fa-angle-double-up')){
      this.$el.find('.vehicle_details_info').hide();
      $btn.addClass('fa-angle-double-down');
      $btn.removeClass('fa-angle-double-up');
    }else{
      this.$el.find('.vehicle_details_info').show();
      $btn.addClass('fa-angle-double-up');
      $btn.removeClass('fa-angle-double-down');
    }
  },
  ...
  vehicleChange: function(vehicle, customer){/*reset the vehicle and customer, case we need it to calcute*/
    if(!vehicle && !customer){/* reset and clear all things */
      ...
      this.vehicle_situation.set_mileage(vehicle.attributes.detail);
    }
