Mis.Views.PosOrderIndexView = Backbone.View.extend({
  ...
  events: {
    ...
    "change input[name='license_number']": "setVehicleInfoData",
  ...
  showAddVehicleForm: function(e){
    ...
    this.checkPhoneNumber(phone_number, function(response){
        new Mis.Views.NewCustomerView({customer: response.customer, license_number: license_number});
    })
  },
  ...
  showVehicleButton: function(){
    ...
    this.checkPhoneNumber(phone_number, function(response){
      if(response.success) {
        $("input[name='customer_name']").val(response.customer.full_name);
        $('.add_new_vehicle_or_customer').show().text("添加车辆");
      }
    })
  },
  ...
  setVehicleInfoData: function(e){
    ...
    if (license_number != "") {
      $.get("/api/store_vehicles/search", {license_number: license_number}, function(data){
        if(data.vehicle_id){
          $('.add_new_vehicle_or_customer').hide();
          _this.storeVehicle.id = data.vehicle_id;
          _this.storeVehicle.fetch({
            success: function(data){
              Mis.Vues.VechileInfo.vehicleInfoData = null;
              Mis.Vues.VechileInfo.vehicleInfoData = _this.storeVehicle.attributes;
              $("input[name='phone_number']").val(_this.storeVehicle.attributes.store_customer.phone_number);
          ...
          _this.storeVehicle = new Mis.Models.StoreVehicle();
          Mis.Vues.VechileInfo.vehicleInfoData = _this.storeVehicle.attributes;
          if($("input[name='phone_number']").val()) {
            _this.showVehicleButton();
          }else{
            $('.add_new_vehicle_or_customer').show().text("添加客户");
          }
        }
      })
    }
