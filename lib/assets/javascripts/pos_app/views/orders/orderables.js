   ...
   var OrderablesView = Backbone.View.extend({
    ...
    toggleServiceItem: function(evt){
      ...
      if(target.checked){
        ...
        var _attrs = service.toItemAttrs(this.customer.attributes);
    ...
    toggleMaterialItem: function(evt){
      ...
      if(target.checked){
        ...
        var _attrs = material.toItemAttrs(this.customer.attributes)
    ...
    togglePackageItem: function(evt){
      ...
      if(target.checked){
        ...
        $(target.parentElement.parentElement.parentElement.nextElementSibling).show();
        this.orderPackagedMaterials(package);
    ...
    orderPackagedMaterials: function(package){
      if(package.attributes.materials){
        package.attributes.materials.forEach(function(material, idx){
          
          var _attrs = {
            name: material.name,
            speci: material.speci,
            orderable_type: material.package_itemable_type,
            orderable_id: material.package_itemable_id,
            retail_price: material.retail_price,
            vip_price: material.price,
            price: 0,
            cached_price: 0,
    ...
    togglePackageServiceItem: function(evt){
      ...
      if(target.checked){
        ...
        var idx = Mis.Vues.ServiceItem.items.findIndex(function(item){
          return item.from_customer_asset && !item.store_customer_asset_item_id && item.package_item_type == 'StorePackageItem' && item.package_item_id == id;
        });
        Mis.Vues.ServiceItem.removeItem(idx);
      }
    },
    ...
    toggleAssetServiceItem: function(evt){
      ...
      if(target.checked){
        var _attrs = {
          ...
          retail_price: asset_item.retail_price,
          vip_price: asset_item.price,
          price: 0,
          quantity: 1,
          from_customer_asset: true
        };
    ...
    showItem: function(e){
      if(e.target.dataset.target == '.js-orderables-assets'){
        this.showAssets();
      }else if (e.target.dataset.target == '.js-zidingyi'){
        this.showZiDingYi();
      } else{
    ...
    showZiDingYi: function(){
      if(this.customer){
        this.hideOtherTableItem();
        if(!this.zidingyi_view){
          this.zidingyi_view = new Mis.ziDingYiView();
        }
        this.zidingyi_view.show();
      }
      else {
        $.alert({text: "先选择车辆或者客户"});
      }
    },
    ...
    showAssets: function(){
      if(this.customer){
        if(this.customer.id){
          if(this.customer_assets && this.customer_assets.id == this.customer.id){
            this.displayAssets();
          }else{
            this.refreshAssets();
          }
        }else{
          this.newCustomerHasNoAssets();
        }
      }else{
        $.alert({text: "先选择车辆或者客户"});
      }
    ...
    renderAssets: function(){
      var html = assets_tmpl(this.customer_assets);
      $('#js-orderables-assets').html(html);
      this.displayAssets();
    },

    displayAssets: function(){
      this.checkSelectedAssetItem();
      this.hideOtherTableItem();
      $('#js-show-orderable-assets').prop('class', 'bg-color-8E81B8 color-fff')
      $('#js-orderables-assets').show();
      $('.js-orderables-assets').show();
    },

    newCustomerHasNoAssets: function(){
      this.customer_assets = {assets: []}
      this.renderAssets();
    },

    refreshAssets: function(){
      var customer_id = this.customer.id;
      if(!!customer_id){
