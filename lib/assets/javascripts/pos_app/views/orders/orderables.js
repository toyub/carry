(function(win, Backbone, $, Mis){
   ...
   var OrderablesView = Backbone.View.extend({
    ...
    events: {
      ...
      'click .js-orderables-assets .js-chose-asset': 'choseAsset'
    },
    initialize: function(){
    ...
    choseAsset: function(evt){
      var _this = this;
      var checkedAssets = $(evt.target).parents('table').find('input:checked');
      checkedAssets.each(function(idx, el){
        
        var asset = _this.customer_assets.assets.find(function(asset){return asset.id == el.dataset.assetid});
        var item = asset.items.find(function(item){return item.id == el.dataset.assetitem});
        var _attrs = {
          name: item.name,
