(function(win, doc, Backbone, $$MIS){
  ...
  var CashierOrderDetailView = Backbone.View.extend({
    ...
    events: {
      ...
      'mousedown select': 'show_select',
      'keydown select': 'show_select'
    },
    ...
    render: function(){
      ...
      this.payments.add(PaymentMethods.available_methods()[0])
    },
    ...
    show_select: function(evt){
      console.log(evt)
    },

    checkout: function(evt){
      evt.preventDefault();
      var paid_amount = this.payments.map(function(model){ return model.get('amount');}).reduce(function(x,y){return x+y});
      var _this = this;
        ...
        this.model.fetch({
          success: function(){
            if(_this.model.hasChanged()){
              ...
              var data = JSON.stringify({order_id: _this.model.get('id'), payments: _this.payments.as_json()});
    ...
    combine_payment: function(){
      var ids = this.payments.map(function(m){
          return m.get('payment_method_type');
        });
